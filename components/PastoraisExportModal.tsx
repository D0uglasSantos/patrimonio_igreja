...
export function PastoraisExportModal() {
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true)
      const response = await fetch('/api/pastorais/export')
      if (!response.ok) {
        const error = (await response.json()) as { error?: string }
        throw new Error(error.error || 'Erro ao exportar pastorais')
      }
      ...
      anchor.href = url
      anchor.download = `pastorais_${Date.now()}.xlsx`
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[480px] border-[#c4c6cf]">
        <DialogHeader>
          <DialogTitle className="text-[#002045]">Exportar Pastorais em Excel</DialogTitle>
          <DialogDescription>
            Exporta todas as pastorais cadastradas no momento.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-xl border border-dashed border-[#c4c6cf] p-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs text-[#74777f]">
              O arquivo incluirá nome da pastoral, total de membros, coordenadores, vice-coordenadores e empréstimos ativos.
            </p>
          </div>
        </div>
