...
export function UsuariosImportModal({ onImportSuccess }: UsuariosImportModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const response = await fetch('/api/usuarios/import/template')
      if (!response.ok) {
      ...
      anchor.href = url
      anchor.download = 'template_importacao_usuarios.xlsx'
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (error.emails && error.emails.length > 0) {
          toast.error('Emails já cadastrados no sistema', {
            description: error.emails.slice(0, 5).join(', '),
          })
          return
        }
      ...
      toast.success(result.message || 'Importação concluída', {
        description: `${result.totalImportados ?? 0} usuários importados com sucesso`,
      })
      setOpen(false)
      resetState()
      onImportSuccess()
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-[640px] border-[#c4c6cf]">
        <DialogHeader>
          <DialogTitle className="text-[#002045]">Importação em Massa de Usuários</DialogTitle>
          <DialogDescription>
            Use o template oficial e importe até 3000 usuários por arquivo.
          </DialogDescription>
        </DialogHeader>
