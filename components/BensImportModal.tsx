...
export function BensImportModal({ onImportSuccess }: BensImportModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const response = await fetch('/api/bens/import/template')
      if (!response.ok) {
      ...
      anchor.href = url
      anchor.download = 'template_importacao_bens.xlsx'
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/bens/import', {
        method: 'POST',
        body: formData,
      })
      ...
      if (!response.ok) {
        ...
        if (error.codigos && error.codigos.length > 0) {
          toast.error('Códigos já cadastrados no sistema', {
            description: error.codigos.slice(0, 5).join(', '),
          })
          return
        }
      ...
      toast.success(result.message || 'Importação concluída', {
        description: `${result.totalImportados ?? 0} bens importados com sucesso`,
      })
      setOpen(false)
      resetState()
      onImportSuccess()
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-[640px] border-[#c4c6cf]">
        <DialogHeader>
          <DialogTitle className="text-[#002045]">Importação em Massa de Bens</DialogTitle>
          <DialogDescription>
            Use o template oficial e importe até 3000 linhas de bens por arquivo.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-dashed border-[#c4c6cf] p-4">
            ...
            <p className="text-xs text-[#74777f] mt-2">
              Limites: até 3000 linhas de bens e tamanho máximo de 10MB.
            </p>
          </div>
        ...
        <DialogFooter>
          ...
          >
            {isUploading ? 'Importando...' : 'Importar Bens'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
